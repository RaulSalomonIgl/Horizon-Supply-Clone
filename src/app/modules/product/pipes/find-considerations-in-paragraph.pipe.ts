...
@Pipe({
  name: 'findConsiderationsInParagraph',
  standalone: true,
})
export class FindConsiderationsInParagraphPipe implements PipeTransform {
  transform(value: string): string {
    // Expresión regular para encontrar el contenido dentro de las etiquetas <p>...</p>
    const regex = /<p[^>]*>(.*?)<\/p>/g;

    // Array para almacenar las coincidencias que contienen 'Wash'
    ...
    matches.forEach((match) => {
      ...
      if (paragraphContent.includes('Wash')) {
